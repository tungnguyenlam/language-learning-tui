...
- [x] **Bug Hunt: Cram Revealed Enter/Space Grade, Space Key Selection & Settings Template Map Safety:** Fixed Cram mode Enter/Space keys when revealed so pressing Enter or Space grades the card as Good (calling `gradeCramCard(core.GradeGood)`); fixed Space key handling in Decks and Browser views to accept `"space"` (Bubble Tea v2 string) and enable multi-select parity in Browser; added nil map safety guard and status feedback when editing AI template settings when `aiTemplates` is uninitialized or `activeSet` is empty. Added regression unit tests `TestCramMultiCardProgression`, `TestSpaceKeySelectionParity`, and `TestSettingsTemplateEditNilMapSafety`.
