...
- [x] **Persistent Dictionary Search History:** Implemented persistent search history for Offline Dictionary searches, storing recent searches in the SQLite `app_settings` table using key `'dict_search_history'`.
 The history is retrieved asynchronously on startup and saved on additions and clearing (`ctrl+x`), maintaining state across application sessions. Added unit coverage `TestDictionaryPersistentSearchHistory`.
