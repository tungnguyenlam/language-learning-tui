...
def test_a2_work_office_deck_exists():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            # Search for Work & Office deck
            agent.act("/")
            agent.wait_for_text("Search:", timeout=2.0)
            agent.act("Work & Office")
            agent.act("<Enter>")

            # Just assert it exists in the list
            agent.wait_for_text("A2 Work & Office", timeout=10.0)

        finally:
            agent.close()

if __name__ == "__main__":
    test_a2_work_office_deck_exists()
